...
export function MovementForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (movementType === "OUT" && qty > selectedProduct.stockCurrent) {
      setError(
        t("movements.error.insufficientStock")
          .replace("{current}", String(selectedProduct.stockCurrent))
      );
      return;
    }
        ...
        <div>
          ...
          <div className="mt-2 grid grid-cols-3 gap-2">
            ...
            >
              ...
              <span className="text-sm font-medium">{t("movements.type.ADJUSTMENT")}</span>
            </button>
          </div>
        </div>
